...
function getHiddenColumns() {
    let hidden = ['Delete', 'delete', 'UPDATE', 'Update', 'update', 'DELETE', 'Path file', 'file path', 'Pathfile', 'path_file', 'Đường dẫn file', 'Who delete'];
    const role = CURRENT_USER && CURRENT_USER.role ? CURRENT_USER.role.toLowerCase() : '';
    if (role !== 'admin app') {
        hidden.push('App_pass', 'app_pass', 'App Pass', 'Mật khẩu', 'Mật khẩu ứng dụng');
    }
    return hidden;
}
        ...
        if (sheet === 'admin' && data === 'admin') {
            return {
                ...
                user: {
                    ...
                    role: 'admin app',
                    'Họ và tên': 'Quản trị viên App',
                    'Phân quyền': 'admin app'
                }
            };
        }
...
function renderDashboard() {
    ...
    const isAdminApp = CURRENT_USER && CURRENT_USER.role && CURRENT_USER.role.toLowerCase() === 'admin app';
    
    if (isAdmin || isAdminApp) {
        kpiHTML += `
...
function renderInventoryTable(data) {
    ...
    // Lấy danh sách cột hiển thị đầy đủ từ Tonkho (trừ getHiddenColumns())
    var columnKeysMap = {};
    rows.forEach(function (r) {
        Object.keys(r).forEach(function (k) {
            if (!getHiddenColumns().includes(k)) columnKeysMap[k] = true;
        });
    });
...
async function processIndividualPayroll(params) {
    ...
    try {
        ...
        var monthlyTransactions = transactionData.filter(t => {
            ...
            return transDate.getMonth() + 1 === parseInt(params.month) && 
                   transDate.getFullYear() === parseInt(params.year) &&
                   (t['ID_NhanVien'] === params.empId || t['MaNhanVien'] === params.empId);
        });
                ...
                <div class="card border-primary">
                    ...
                    <div class="card-body">
                        <div class="row g-2">
                            ...
                            <div class="col-6 text-end text-warning fw-bold money-display">${formatMoney(allowance, true)}</div>
                            
                            <div class="col-6"><strong>Thưởng:</strong></div>
                            <div class="col-6 text-end text-success fw-bold money-display">${formatMoney(bonus, true)}</div>
                            
                            <div class="col-6"><strong>Tạm ứng:</strong></div>
                            <div class="col-6 text-end text-danger fw-bold money-display">-${formatMoney(advance, true)}</div>
                            
                            <hr class="my-2">
                            
                            <div class="col-6"><strong style="font-size: 16px;">THỰC LĨNH:</strong></div>
                            <div class="col-6 text-end fw-bold text-primary money-display" style="font-size: 18px;">${formatMoney(netSalary, true)}</div>
                        </div>
                        
                        <div class="mt-3 text-center">
                            <button class="btn btn-success btn-sm me-2" onclick="Swal.close(); setTimeout(() => saveIndividualPayroll(${JSON.stringify(payrollRecord).replace(/"/g, '&quot;')}), 200);">
...
// --- LƯU LƯƠNG CÁ NHÂN ---
window.saveIndividualPayroll = async function(payrollRecord) {
    try {
        showLoading(true, 'Đang lưu lương...');
        
        var existingPayroll = GLOBAL_DATA['BangLuongThang'] || [];
        var existingRecord = existingPayroll.find(p => p['ID_BangLuong'] === payrollRecord['ID_BangLuong']);
        ...
        if (result.status === 'success') {
            ...
            Swal.fire({
                icon: 'success',
                title: 'Đã lưu lương!',
                text: `Lương của ${payrollRecord['HoTen']} đã được lưu vào bảng lương tháng ${payrollRecord['Thang']}/${payrollRecord['Nam']}`,
                confirmButtonColor: '#28a745'
            });
        } else {
...
function renderTable(data) {
    ...
    var table = document.getElementById('data-table');
    if (table) {
        table.classList.remove('attendance-table');
        table.classList.remove('payroll-table');
        table.classList.remove('user-table');
        if (CURRENT_SHEET === 'User') {
    ...
    // Đảm bảo cột App_pass luôn hiển thị cho admin app trong bảng User
    if (CURRENT_SHEET === 'User') {
        const role = CURRENT_USER && CURRENT_USER.role ? CURRENT_USER.role.toLowerCase() : '';
        if (role === 'admin app' && !keys.some(k => k.toLowerCase() === 'app_pass' || k.toLowerCase() === 'app pass' || k.toLowerCase() === 'mật khẩu' || k.toLowerCase() === 'mật khẩu ứng dụng')) {
            keys.push('App_pass');
        }
    }

    // Sắp xếp cột cho User sheet để ID và Họ và tên luôn đứng đầu
    if (CURRENT_SHEET === 'User') {
        const priorityKeys = ['ID', 'Họ và tên'];
        keys.sort((a, b) => {
            let idxA = priorityKeys.indexOf(a);
            let idxB = priorityKeys.indexOf(b);
    ...
    keys.forEach(k => { 
        ...
        if (CURRENT_SHEET === 'User') {
            if (k === 'ID') t.classList.add('user-sticky-col-1');
            if (k === 'Họ và tên') t.classList.add('user-sticky-col-2');
        }
        trH.appendChild(t); 
    });
    ...
    activeData.forEach((r) => {
        ...
        keys.forEach(k => {
            if (!skip.includes(k)) {
                var td = document.createElement('td');
                if (CURRENT_SHEET === 'User') {
                    if (k === 'ID') td.classList.add('user-sticky-col-1');
                    if (k === 'Họ và tên') td.classList.add('user-sticky-col-2');
                }
                var v = r[k];
...
function buildForm(s, d) {
    ...
    // Đảm bảo cột App_pass luôn có trong form cho admin app
    if (s === 'User') {
        const role = CURRENT_USER && CURRENT_USER.role ? CURRENT_USER.role.toLowerCase() : '';
        if (role === 'admin app' && !Object.keys(smp).some(k => k.toLowerCase() === 'app_pass' || k.toLowerCase() === 'app pass' || k.toLowerCase() === 'mật khẩu' || k.toLowerCase() === 'mật khẩu ứng dụng')) {
            smp = { ...smp, 'App_pass': '' };
        }
    }
    ...
    Object.keys(smp).forEach(k => {
        ...
        if (drop && GLOBAL_DATA[drop.table]) {
            ...
            // Handle readonly for ID and App_pass
            const role = CURRENT_USER && CURRENT_USER.role ? CURRENT_USER.role.toLowerCase() : '';
            if (role !== 'admin app') {
                if (k.toLowerCase() === 'id' || k.toLowerCase() === 'app_pass' || k.toLowerCase() === 'app pass' || k.toLowerCase() === 'mật khẩu' || k.toLowerCase() === 'mật khẩu ứng dụng') {
                    inputAttrs += ' readonly';
                }
            }
...
function showRowDetail(r, s, idx) {
    ...
    var skip = getHiddenColumns().concat(['ID_Phieu', 'ID phieu nhap', 'ID phieu Xuat', 'ID Chkho']);
    for (var k in r) {
...
window.viewProfile = function () {
    ...
    let detailsHtml = '';
    const excludeKeys = ['ID', 'Họ và tên', 'Phân quyền', 'Chức vụ', 'App_pass', 'app_pass', 'App Pass', 'Mật khẩu', 'Mật khẩu ứng dụng', 'name', 'role', 'Delete'];
    
    for (const [key, value] of Object.entries(CURRENT_USER)) {
        if (!excludeKeys.map(k => k.toLowerCase()).includes(key.toLowerCase()) && value !== null && value !== undefined && value !== '') {
            detailsHtml += `
    ...
    var html = `
        <div class="text-center mb-4">
            ...
            <span class="badge bg-secondary px-3 py-2 rounded-pill">${CURRENT_USER.role && CURRENT_USER.role.toLowerCase() === 'admin app' ? 'Quản trị viên App' : CURRENT_USER.role && CURRENT_USER.role.toLowerCase().includes('admin') ? 'Quản trị viên' : 'Nhân viên'}</span>
        </div>
